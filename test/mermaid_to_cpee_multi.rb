...
sets.each do |s|
  ...
  cpee_dir = File.join(root_dir,'cpee')
  mer_dir = File.join(root_dir,'mermaid_2')
  Dir.mkdir(cpee_dir) unless File.exist?(cpee_dir)
  Dir.mkdir(mer_dir) unless File.exist?(mer_dir)
  Dir.foreach(root_dir) do |file_name|
    ...
    if new_mermaid == 0
      ...
      File.write(File.join(cpee_dir,file_name), cpee_tree)
      File.write(File.join(mer_dir,file_name), new_mermaid)
